  ...
  try {
    ...
    if (Array.isArray(body)) {
      ...
      const spans = validatedSpans.map(span => spansDb.create({
        ...
        input_data: span.inputData ? JSON.stringify(span.inputData) : null,
        output_data: span.outputData ? JSON.stringify(span.outputData) : null,
        ...
        token_usage: span.tokenUsage ? JSON.stringify(span.tokenUsage) : null,
        cost: span.cost || null,
    ...
    // Single span
    const validatedData = spanSchema.parse(body)
    ...
    const span = spansDb.create({
      ...
      token_usage: validatedData.tokenUsage ? JSON.stringify(validatedData.tokenUsage) : null,
