...
export class AuthService {
  ...
  static async generateJwtToken(user: any, sessionId: string): Promise<string> {
    try {
      console.log('=== JWT GENERATION DEBUG START ===');
      console.log('User object:', JSON.stringify(user, null, 2));
      console.log('SessionId:', sessionId);
      console.log('JWT_SECRET exists:', !!this.JWT_SECRET);
      console.log('JWT_SECRET length:', this.JWT_SECRET?.length || 'undefined');
      console.log('JWT_SECRET type:', typeof this.JWT_SECRET);
      console.log('JWT_EXPIRE_TIME:', config.JWT_EXPIRE_TIME);
      console.log('SESSION_EXPIRE_TIME:', this.SESSION_EXPIRE_TIME);
      
      authLogger.debug(`Generating JWT for user:`, {
        userId: user.id,
        username: user.username,
        role: user.role,
        workspaceId: user.workspaceId,
        sessionId,
        jwtSecret: this.JWT_SECRET ? 'SET' : 'NOT_SET',
        jwtExpireTime: config.JWT_EXPIRE_TIME,
        sessionExpireTime: this.SESSION_EXPIRE_TIME
      });
      ...
      authLogger.debug('JWT payload prepared:', payload);

      const signOptions = {
        expiresIn: `${this.SESSION_EXPIRE_TIME}s`,
      };

      console.log('JWT options:', JSON.stringify(signOptions, null, 2));
      console.log('About to call jwt.sign...');
      const token = jwt.sign(payload, this.JWT_SECRET, signOptions);
      console.log('jwt.sign completed successfully');
      console.log('Token length:', token?.length);

      authLogger.debug(`JWT token generated successfully for user ${user.id}`);
      console.log('=== JWT GENERATION DEBUG END ===');
      return token;
    } catch (error) {
      console.log('=== JWT GENERATION ERROR DEBUG ===');
      ...
      console.log('JWT_SECRET exists:', !!this.JWT_SECRET);
      console.log('JWT_SECRET type:', typeof this.JWT_SECRET);
      console.log('=== JWT GENERATION ERROR DEBUG END ===');
      
      authLogger.error('JWT generation failed with detailed error:', {
        error: error.message,
        stack: error.stack,
        user: {
          id: user?.id,
