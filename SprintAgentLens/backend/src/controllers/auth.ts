...
export default async function authController(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/login', {
    ...
    try {
      ...
      console.log('Request received, starting validation...');
      ...
      console.log('=== AUTHSERVICE.AUTHENTICATE COMPLETED ===');
      console.log('authResult:', authResult);
      console.log('authResult.success:', authResult.success);
      console.log('authResult.user:', authResult.user);
      console.log('authResult.token:', authResult.token ? `${authResult.token.substring(0, 20)}...` : 'null');
      ...
      try {
        console.log('=== AUTH CONTROLLER DEBUG START ===');
        console.log('authResult:', authResult);
        console.log('config.SESSION_EXPIRE_TIME:', config.SESSION_EXPIRE_TIME);
        ...
        console.log('loginResponse prepared:', loginResponse);

        // Set HTTP-only cookie for additional security
        try {
          reply.cookie('auth-token', authResult.token!, {
            httpOnly: true,
          ...
          console.log('Cookie set failed, but continuing without cookie:', cookieError);
          // Continue without cookie - the token in response body is sufficient
        }
        ...
        const result = reply.status(200).send(loginResponse);
        console.log('Response sent successfully');
        console.log('=== AUTH CONTROLLER DEBUG END ===');
        return result;
      } catch (innerError) {
        console.log('=== AUTH CONTROLLER ERROR DETAILS ===');
        console.log('Inner error in response preparation:', innerError);
        console.log('Error name:', innerError instanceof Error ? innerError.name : 'Unknown');
        console.log('Error message:', innerError instanceof Error ? innerError.message : 'Unknown');
        console.log('Error stack:', innerError instanceof Error ? innerError.stack : 'Unknown');
        console.log('=== AUTH CONTROLLER ERROR END ===');
        throw innerError;
      }
    } catch (error) {
    ...
    try {
      ...
      if (success) {
        ...
        try {
          ...
          // Cookie clearing failed but logout still succeeded
          console.log('Cookie clear failed, but logout succeeded:', cookieError);
        }
